Polymer({
  ...
  _viewChanged: function() {
    ...
    if (this.minutes) {
      ...
      this._hour12draw(this.hour12);
      this._hour24draw(this.hour12);
    }
  },
  ...
  _valueChanged: function() {
    ...
    if (this.value < 0 || this.value > (this.minutes ? ticks.length - 1 : 23)) {
      this.value = 0;
    }
    this._selectorPos();
  ...
  _selectorPos: function() {
    ...
    var tickIndx = this.minutes ? this.value : this.value % (this.hour12 ? 12 : 24);
    ticks[tickIndx].classList.toggle('select');
  ...
  _toggleMeridiem: function(evt) {
    evt.stopPropagation();
    if (evt.currentTarget.classList.contains('select')) return;
    this.$[this.meridiem].classList.toggle('select');
    this.meridiem = evt.currentTarget.classList.contains('pm') ? 'PM' : 'AM';
    this.$[this.meridiem].classList.toggle('select');
    switch (this.meridiem) {
      case "AM":
        this.value -= (this.value > 12) ? 12 : 0;
        break;
      case "PM":
        this.value += (this.value < 12) ? 12 : 0;
        break;
  ...
  _dragSelect: function(evt) {
    ...
    this.value = indx + (this.minutes || this.hour12 ? 0 : (coord.r > this._radius - 20) ? 0 : 12);
  },
